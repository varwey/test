 ...
 */
define("nowdo/summernote/0.5.2/summernote-debug", [ "$-debug", "./summernote-zh-CN-debug" ], function(require, exports) {
    (function(factory) {
        /* global define */
        if (typeof define === "function" && define.amd) {
            // AMD. Register as an anonymous module.
            define([ "jquery", "codemirror" ], factory);
        } else if (typeof define === "function" && typeof exports === "object") {
            var $ = require("$-debug");
     ...
     */
       ...
       */
            options: {
                ...
                // set editor width
                height: null,
                // set editable height, ex) 300
                focus: false,
                // set focus after initilize summernote
                tabsize: 4,
                ...
                // toolbar
                toolbar: [ [ "style", [ "style" ] ], [ "font", [ "bold", "italic", "underline", "clear" ] ], [ "fontname", [ "fontname" ] ], // ['fontsize', ['fontsize']], // Still buggy
                [ "color", [ "color" ] ], [ "para", [ "ul", "ol", "paragraph" ] ], [ "height", [ "height" ] ], [ "table", [ "table" ] ], [ "insert", [ "link", "picture", "video" ] ], [ "view", [ "fullscreen", "codeview" ] ], [ "help", [ "help" ] ] ],
                ...
                // fontName
                fontNames: [ "Serif", "Sans", "Arial", "Arial Black", "Courier", "Courier New", "Comic Sans MS", "Helvetica", "Impact", "Lucida Grande", "Lucida Sans", "Tahoma", "Times", "Times New Roman", "Verdana" ],
       ...
       */
            var textRange2bp = function(textRange, bStart) {
                ...
                if (nOffset !== 0 && dom.isText(aChild[nOffset - 1])) {
                    ...
                    /* jshint ignore:start */
                    var sDummy = elCurText.nodeValue;
                    //enforce IE to re-reference elCurText, hack
                    /* jshint ignore:end */
                    if (bStart && elCurText.nextSibling && dom.isText(elCurText.nextSibling) && nTextCount === elCurText.nodeValue.length) {
                        nTextCount -= elCurText.nodeValue.length;
                        elCurText = elCurText.nextSibling;
                    }
                    elCont = elCurText;
                ...
                create: function(sc, so, ec, eo) {
                    if (arguments.length === 0) {
                        ...
                        if (bW3CRangeSupport) {
                            ...
                            var bpStart = textRange2bp(textRangeStart, true), bpEnd = textRange2bp(textRangeEnd, false);
                            sc = bpStart.cont;
                            so = bpStart.offset;
                            ec = bpEnd.cont;
                            eo = bpEnd.offset;
     ...
     */
       ...
       */
            ...
            var aCmd = [ "bold", "italic", "underline", "strikethrough", "justifyLeft", "justifyCenter", "justifyRight", "justifyFull", "insertOrderedList", "insertUnorderedList", "indent", "outdent", "formatBlock", "removeFormat", "backColor", "foreColor", "insertHorizontalRule", "fontName" ];
       ...
       */
            this.insertVideo = function($editable, sUrl) {
                ...
                if (ytMatch && ytMatch[2].length === 11) {
                    ...
                    $video = $("<iframe>").attr("src", igMatch[0] + "/embed/").attr("width", "612").attr("height", "710").attr("scrolling", "no").attr("allowtransparency", "true");
                } else if (vMatch && vMatch[0].length > 0) {
                    $video = $("<iframe>").attr("src", vMatch[0] + "/embed/simple").attr("width", "600").attr("height", "600").attr("class", "vine-embed");
                } else if (vimMatch && vimMatch[3].length > 0) {
                    $video = $("<iframe webkitallowfullscreen mozallowfullscreen allowfullscreen>").attr("src", "//player.vimeo.com/video/" + vimMatch[3]).attr("width", "640").attr("height", "360");
                } else if (dmMatch && dmMatch[2].length > 0) {
                    $video = $("<iframe>").attr("src", "//www.dailymotion.com/embed/video/" + dmMatch[2]).attr("width", "640").attr("height", "360");
                } else {}
                if ($video) {
            ...
            this.update = function($handle, oStyle) {
                ...
                if (oStyle.image) {
                    ...
                    $selection.css({
                        ...
                        width: szImage.w,
                        height: szImage.h
                    }).data("target", oStyle.image);
                    // save current image element.
       ...
       */
            this.showVideoDialog = function($editable, $dialog, videoInfo) {
                return $.Deferred(function(deferred) {
                    ...
                    var $videoUrl = $videoDialog.find(".note-video-url"), $videoBtn = $videoDialog.find(".note-video-btn");
     ...
     */
       ...
       */
            var hPasteClipboardImage = function(event) {
                ...
                if (!originalEvent.clipboardData || !originalEvent.clipboardData.items || !originalEvent.clipboardData.items.length) {
                    return;
                }
                var oLayoutInfo = makeLayoutInfo(event.currentTarget || event.target);
       ...
       */
            var hHandleMousedown = function(event) {
                if (dom.isControlSizing(event.target)) {
                    ...
                    var oLayoutInfo = makeLayoutInfo(event.target), $handle = oLayoutInfo.handle(), $popover = oLayoutInfo.popover(), $editable = oLayoutInfo.editable();
                    var elTarget = $handle.find(".note-control-selection").data("target"), $target = $(elTarget), posStart = $target.offset(), scrollTop = $document.scrollTop();
            ...
            var toggleFullscreen = function(oLayoutInfo) {
                var $editor = oLayoutInfo.editor(), $toolbar = oLayoutInfo.toolbar(), $editable = oLayoutInfo.editable(), $codable = oLayoutInfo.codable();
            ...
            var hToolbarAndPopoverClick = function(event) {
                ...
                if ($btn.length > 0) {
                    ...
                    var $dialog = oLayoutInfo.dialog(), $editable = oLayoutInfo.editable();
       ...
       */
            var attachDragAndDropEvent = function(oLayoutInfo) {
                var collection = $(), $dropzone = oLayoutInfo.dropzone, $dropzoneMessage = oLayoutInfo.dropzone.find(".note-dropzone-message");
     ...
     */
       ...
       */
            var tplButton = function(sLabel, options) {
                ...
                return '<button type="button"' + ' class="btn btn-default btn-sm btn-small' + (className ? " " + className : "") + (dropdown ? " dropdown-toggle" : "") + '"' + (dropdown ? ' data-toggle="dropdown"' : "") + (title ? ' title="' + title + '"' : "") + (event ? ' data-event="' + event + '"' : "") + (value ? " data-value='" + value + "'" : "") + ' tabindex="-1">' + sLabel + (dropdown ? ' <span class="caret"></span>' : "") + "</button>" + (dropdown || "");
            };
            /**
       ...
       */
            var tplPopover = function(className, content) {
                return '<div class="' + className + ' popover bottom in" style="display: none;">' + '<div class="arrow"></div>' + '<div class="popover-content">' + content + "</div>" + "</div>";
            };
            /**
       ...
       */
            ...
            var tplButtonInfo = {
                ...
                table: function(lang) {
                    var dropdown = '<ul class="dropdown-menu">' + '<div class="note-dimension-picker">' + '<div class="note-dimension-picker-mousecatcher" data-event="insertTable" data-value="1x1"></div>' + '<div class="note-dimension-picker-highlighted"></div>' + '<div class="note-dimension-picker-unhighlighted"></div>' + "</div>" + '<div class="note-dimension-display"> 1 x 1 </div>' + "</ul>";
                ...
                style: function(lang, options) {
                    var items = options.styleTags.reduce(function(memo, v) {
                        ...
                        return memo + '<li><a data-event="formatBlock" data-value="' + v + '">' + (v === "p" || v === "pre" ? label : "<" + v + ">" + label + "</" + v + ">") + "</a></li>";
                    }, "");
                ...
                fontname: function(lang, options) {
                    var items = options.fontNames.reduce(function(memo, v) {
                        return memo + '<li><a data-event="fontName" data-value="' + v + '">' + '<i class="fa fa-check icon-ok"></i> ' + v + "</a></li>";
                    }, "");
                    var sLabel = '<span class="note-current-fontname">' + options.defaultFontName + "</span>";
                ...
                fontsize: function(lang, options) {
                    var items = options.fontSizes.reduce(function(memo, v) {
                        return memo + '<li><a data-event="fontSize" data-value="' + v + '">' + '<i class="fa fa-check icon-ok"></i> ' + v + "</a></li>";
                    }, "");
                ...
                paragraph: function(lang) {
                    ...
                    var dropdown = '<div class="dropdown-menu">' + '<div class="note-align btn-group">' + leftButton + centerButton + rightButton + justifyButton + "</div>" + '<div class="note-list btn-group">' + indentButton + outdentButton + "</div>" + "</div>";
                ...
                height: function(lang, options) {
                    var items = options.lineHeights.reduce(function(memo, v) {
                        return memo + '<li><a data-event="lineHeight" data-value="' + v + '">' + '<i class="fa fa-check icon-ok"></i> ' + v + "</a></li>";
                    }, "");
            ...
            var tplPopovers = function(lang, options) {
                ...
                var tplImagePopover = function() {
                    ...
                    var content = '<div class="btn-group">' + fullButton + halfButton + quarterButton + "</div>" + '<div class="btn-group">' + leftButton + rightButton + justifyButton + "</div>" + '<div class="btn-group">' + removeButton + "</div>";
                ...
                return '<div class="note-popover">' + tplLinkPopover() + tplImagePopover() + (options.airMode ? tplAirPopover() : "") + "</div>";
            };
       ...
       */
            var tplShortcut = function(title, body) {
                return '<table class="note-shortcut">' + "<thead>" + "<tr><th></th><th>" + title + "</th></tr>" + "</thead>" + "<tbody>" + body + "</tbody>" + "</table>";
            };
            var tplShortcutText = function(lang) {
                var body = "<tr><td>⌘ + B</td><td>" + lang.font.bold + "</td></tr>" + "<tr><td>⌘ + I</td><td>" + lang.font.italic + "</td></tr>" + "<tr><td>⌘ + U</td><td>" + lang.font.underline + "</td></tr>" + "<tr><td>⌘ + ⇧ + S</td><td>" + lang.font.strike + "</td></tr>" + "<tr><td>⌘ + \\</td><td>" + lang.font.clear + "</td></tr>";
                return tplShortcut(lang.shortcut.textFormatting, body);
            };
            var tplShortcutAction = function(lang) {
                var body = "<tr><td>⌘ + Z</td><td>" + lang.history.undo + "</td></tr>" + "<tr><td>⌘ + ⇧ + Z</td><td>" + lang.history.redo + "</td></tr>" + "<tr><td>⌘ + ]</td><td>" + lang.paragraph.indent + "</td></tr>" + "<tr><td>⌘ + [</td><td>" + lang.paragraph.outdent + "</td></tr>" + "<tr><td>⌘ + ENTER</td><td>" + lang.hr.insert + "</td></tr>";
                return tplShortcut(lang.shortcut.action, body);
            };
            var tplShortcutPara = function(lang) {
                var body = "<tr><td>⌘ + ⇧ + L</td><td>" + lang.paragraph.left + "</td></tr>" + "<tr><td>⌘ + ⇧ + E</td><td>" + lang.paragraph.center + "</td></tr>" + "<tr><td>⌘ + ⇧ + R</td><td>" + lang.paragraph.right + "</td></tr>" + "<tr><td>⌘ + ⇧ + J</td><td>" + lang.paragraph.justify + "</td></tr>" + "<tr><td>⌘ + ⇧ + NUM7</td><td>" + lang.lists.ordered + "</td></tr>" + "<tr><td>⌘ + ⇧ + NUM8</td><td>" + lang.lists.unordered + "</td></tr>";
                return tplShortcut(lang.shortcut.paragraphFormatting, body);
            };
            var tplShortcutStyle = function(lang) {
                var body = "<tr><td>⌘ + NUM0</td><td>" + lang.style.normal + "</td></tr>" + "<tr><td>⌘ + NUM1</td><td>" + lang.style.h1 + "</td></tr>" + "<tr><td>⌘ + NUM2</td><td>" + lang.style.h2 + "</td></tr>" + "<tr><td>⌘ + NUM3</td><td>" + lang.style.h3 + "</td></tr>" + "<tr><td>⌘ + NUM4</td><td>" + lang.style.h4 + "</td></tr>" + "<tr><td>⌘ + NUM5</td><td>" + lang.style.h5 + "</td></tr>" + "<tr><td>⌘ + NUM6</td><td>" + lang.style.h6 + "</td></tr>";
            ...
            var tplShortcutTable = function(lang, options) {
                var template = '<table class="note-shortcut-layout">' + "<tbody>" + "<tr><td>" + tplShortcutAction(lang, options) + "</td><td>" + tplShortcutText(lang, options) + "</td></tr>" + "<tr><td>" + tplShortcutStyle(lang, options) + "</td><td>" + tplShortcutPara(lang, options) + "</td></tr>";
            ...
            var tplDialogs = function(lang, options) {
                var tplImageDialog = function() {
                    var body = "<h5>" + lang.image.selectFromFiles + "</h5>" + '<input class="note-image-input" type="file" name="files" accept="image/*" />' + "<h5>" + lang.image.url + "</h5>" + '<input class="note-image-url form-control span12" type="text" />';
                ...
                var tplVideoDialog = function() {
                    var body = '<div class="form-group">' + "<label>" + lang.video.url + '</label>&nbsp;<small class="text-muted">' + lang.video.providers + "</small>" + '<input class="note-video-url form-control span12" type="text" />' + "</div>";
                    var footer = '<button href="#" class="btn btn-primary note-video-btn disabled" disabled>' + lang.video.insert + "</button>";
                    return tplDialog("note-video-dialog", lang.video.insert, body, footer);
                };
                var tplHelpDialog = function() {
                    var body = '<a class="modal-close pull-right" aria-hidden="true" tabindex="-1">' + lang.shortcut.close + "</a>" + '<div class="title">' + lang.shortcut.shortcuts + "</div>" + (agent.bMac ? tplShortcutTable(lang, options) : replaceMacKeys(tplShortcutTable(lang, options))) + '<p class="text-center">' + '<a href="//hackerwins.github.io/summernote/" target="_blank">Summernote 0.5.2</a> · ' + '<a href="//github.com/HackerWins/summernote" target="_blank">Project</a> · ' + '<a href="//github.com/HackerWins/summernote/issues" target="_blank">Issues</a>' + "</p>";
                    return tplDialog("note-help-dialog", "", body, "");
                };
                return '<div class="note-dialog">' + tplImageDialog() + tplLinkDialog() + tplVideoDialog() + tplHelpDialog() + "</div>";
            };
                ...
                $container.find(".note-color-palette").each(function() {
                    ...
                    for (var row = 0, szRow = aaColor.length; row < szRow; row++) {
                        ...
                        for (var col = 0, szCol = aColor.length; col < szCol; col++) {
                            ...
                            aButton.push([ '<button type="button" class="note-color-btn" style="background-color:', sColor, ';" data-event="', sEvent, '" data-value="', sColor, '" title="', sColor, '" data-toggle="button" tabindex="-1"></button>' ].join(""));
                        }
    ...
    require("./summernote-zh-CN-debug");
});

define("nowdo/summernote/0.5.2/summernote-zh-CN-debug", [ "$-debug" ], function(require, exports) {
    var $ = require("$-debug");
